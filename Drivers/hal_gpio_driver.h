...
/* Macros to enable clock for different GPIO ports in register*/
#define _HAL_RCC_GPIOA_CLK_ENABLE() (RCC->AHB1ENR |= (1 << 0)) //set 0th bit
#define _HAL_RCC_GPIOB_CLK_ENABLE() (RCC->AHB1ENR |= (1 << 1)) //set 0th bit
#define _HAL_RCC_GPIOC_CLK_ENABLE() (RCC->AHB1ENR |= (1 << 2)) //set 0th bit
#define _HAL_RCC_GPIOD_CLK_ENABLE() (RCC->AHB1ENR |= (1 << 3)) //set 0th bit
#define _HAL_RCC_GPIOE_CLK_ENABLE() (RCC->AHB1ENR |= (1 << 4)) //set 0th bit
#define _HAL_RCC_GPIOF_CLK_ENABLE() (RCC->AHB1ENR |= (1 << 5)) //set 0th bit
#define _HAL_RCC_GPIOG_CLK_ENABLE() (RCC->AHB1ENR |= (1 << 6)) //set 0th bit
#define _HAL_RCC_GPIOH_CLK_ENABLE() (RCC->AHB1ENR |= (1 << 7)) //set 0th bit
...
/* this struct will be filled and passed to driver by the app to init. the gpio pin. */
typedef struct
{
  uint32_t pin;        //which gpio pin is to be configured
  uint32_t mode;       //what operating mode the pin will be
  uint32_t op_type;    //what type of output for the selected pin
  uint32_t pull;       //whether or not to use pull-up/pull-down
  uint32_t speed;      //what speed to use
  uint32_t alternate;  //type of alternate value, if the mode is set for alt function mode.
